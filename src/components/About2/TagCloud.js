...
export default class TagCloud extends React.Component {
	...
	componentDidMount() {
		document.addEventListener('mousemove', (e) => {
			const angleX = 2 * (e.clientX / document.body.getBoundingClientRect().width - .3) * this.state.speed * BASEANGLE;
			const angleY = 2 * (e.clientY / document.body.getBoundingClientRect().height - .3) * this.state.speed * BASEANGLE;
			this.setState({
				angleX,
				angleY
			})
